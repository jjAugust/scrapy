...
#                 time.sleep(5)
                look_more=".//li[@id='itemPropsLi0"+str(request.meta['para0'])+"']"
                driver.find_element_by_xpath(look_more).click()  # 数据由js来控制,点击后加载数据
                look_more=".//li[@id='itemPropsLi1"+str(request.meta['para1'])+"']"
                print(look_more)
                driver.find_element_by_xpath(look_more).click()  # 数据由js来控制,点击后加载数据
#                 look_more=".//li[@id='itemPropsLi20']"
#                 driver.find_element_by_xpath(look_more).click()  # 数据由js来控制,点击后加载数据
                look_more=".//div[@class='month-list']/span["+str(request.meta['para2'])+"]"
                print(look_more)
                driver.find_element_by_xpath(look_more).click()  # 数据由js来控制,点击后加载数据
                look_more=".//div[@class='item-calendar']/table[@class='date-table']/tbody/tr["+str(request.meta['para3'])+"]/td["+str(request.meta['para4'])+"]/div"
                print(look_more)
                driver.find_element_by_xpath(look_more).click()  # 数据由js来控制,点击后加载数据
#                 time.sleep(5)
