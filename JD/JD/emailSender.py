...
class emailSender(object):
    def __init__(self):
        ...
        self.smtp_user = "466301416@qq.com" # 用于登录smtp服务器的用户名，也就是发送者的邮箱
        self.smtp_pwd = "pakxqwrinyywcbai"  # 授权码，和用户名user一起，用于登录smtp， 非邮箱密码
        self.smtp_port = 465                # smtp服务器SSL端口号，默认是465
        self.sender = "466301416@qq.com"    # 发送方的邮箱
    ...
    def sendEmail(self, toLst, subject, body):
        ...
        try:
            ...
            print("登录结果：loginRes = "+str(loginRes))    # loginRes = (235, b'Authentication successful')
            if loginRes and loginRes[0] == 235:
                print("登录成功，code = "+str(loginRes[0]))
                smtpSSLClient.sendmail(self.sender, toLst, message.as_string())
                print("mail has been send successfully. message:"+str(message.as_string(message.as_string())))
            else:
                print("登陆失败，code = "+str(loginRes[0]))
        except Exception as e:
            print("发送失败，Exception: e="+str(e))